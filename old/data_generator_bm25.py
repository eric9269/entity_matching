...
# 讀檔（自行改路徑）
leaf = pd.read_csv("old/all_leaf.csv")
root = pd.read_csv("old/all_root.csv")
...
for row in df_pos.itertuples():
    ...
    if pool is None or len(pool) < 2:
        # 若該平台 pool 太小，跨平台兜底
        any_plat = np.random.choice(list(root_by_plat.keys()))
        pool = root_by_plat[any_plat]
    # 排除 group_id
    pool_filtered = pool[pool['RootSku'] != row.group_id].reset_index(drop=True)
    titles = pool_filtered['RootTitle'].tolist()
    # BM25 tokenize
    tokenized_corpus = [t.split() for t in titles]
    bm25 = BM25Okapi(tokenized_corpus)
    query = row.e1.split()
    scores = bm25.get_scores(query)
    # 取分數最高的 neg_per_pos 個
    top_idx = np.argsort(scores)[::-1][:neg_per_pos]
    for idx in top_idx:
        cand = pool_filtered.iloc[idx]
        neg_rows.append({
            'e1': row.e1,
            'e2': cand['RootTitle'],
            'label': 0,
...
df_all[['e1','e2','label','group_id']].to_csv("old/bm25_data/pairs_all_bm25.csv", index=False)
...
train_df.to_csv("old/bm25_data/pairs_train_bm25.csv", index=False)
test_df.to_csv("old/bm25_data/pairs_test_bm25.csv", index=False)
